                ...
                <form action="{{route('posts.update', ['post'=>$post->id])}}" enctype="multipart/form-data" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="category_id" id="category_id" class="form-control" autofocus>
                            @foreach ($categories as $cat)
                                <option value="{{(int)$cat->id}}" {{ ((int)$cat->id === (int)$post->category->id)? "selected":'' }}>
                                    {{$cat->name}}
                                </option>    
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="content" id="content" 
                            ...
                            minlength="10" required >{{$post->content}}</textarea>
                        @error('content')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="pt-4">
                        ...
                        <button type="submit" name="submitBtn" value="update" 
                            title="Update Post" class="btn btn-primary">
                            Update
                        </button>
        
                        {{-- Cancel Updating --}}
                        <button type="button" onclick="window.history.go(-1); return false;" 
                            title="Cancel Updating" class="btn btn-outline-secondary">
                            Cancel
                        </button>
